...
object TimeUsage {
  ...
  def row(line: List[String]): Row =
    Row(line.head :: line.tail.map( s => s.toDouble) : _*)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedColumnsListBuffer = new ListBuffer[Column]()
    val workingActivitiesColumnsListBuffer = new ListBuffer[Column]()
    val otherActivitiesColumnsListBuffer = new ListBuffer[Column]()
    columnNames.foreach(
      c => if (
        c.startsWith("t01") || c.startsWith("t03") || c.startsWith("t11")
          || c.startsWith("t1801") || c.startsWith("t1803")
      ) {
        primaryNeedColumnsListBuffer += new Column(c)
      }
      else if (c.startsWith("t05") || c.startsWith("t1805")) {
        workingActivitiesColumnsListBuffer += new Column(c)
      }
      else if (
        c.startsWith("t02") || c.startsWith("t04") || c.startsWith("t06") || c.startsWith("t07")
          || c.startsWith("t08") || c.startsWith("t09") || c.startsWith("t10") || c.startsWith("t12")
          || c.startsWith("t13") || c.startsWith("t14") || c.startsWith("t15") || c.startsWith("t16")
          || c.startsWith("t18")
      ) {
        otherActivitiesColumnsListBuffer += new Column(c)
      }
    )

    (primaryNeedColumnsListBuffer.toList, workingActivitiesColumnsListBuffer.toList, otherActivitiesColumnsListBuffer.toList)
  }
