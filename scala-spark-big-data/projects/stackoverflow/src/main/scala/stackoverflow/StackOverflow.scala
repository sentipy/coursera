...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    val questions = cachedPostings
      .filter(_.postingType == 1)
      .map( p => (p.id, p) )
    val answers = cachedPostings
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    def answerHighScoreOptimized(as: TraversableOnce[Posting]): Int = {
      var highScore = 0
      val it = as.toIterator
      while (it.hasNext) {
        val score = it.next().score
        if (score > highScore)
          highScore = score
      }
      highScore
    ...
    grouped
      .values
      .map(
        e => (e.head._1, answerHighScoreOptimized(e.map(_._2)))
      )
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored
      .map(
        e => (firstLangInTag(e._1.tags, langs), e._2)
      )
      .filter(
        e => e._1.isDefined
      )
      .map(
        e => (langSpread * e._1.get, e._2)
      )
      .cache()
  }
