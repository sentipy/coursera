...
func readFileWeek3(file string) *knapsack.KnapsackProblem {
	f, err := os.Open(file)
	if err != nil {
		return nil
	}
...
func parseFileWeek3(file *os.File) *knapsack.KnapsackProblem {
...
func main() {
	...
	knapsackProblem := readFileWeek3(file)
	result := knapsack.SolveKnapsackProblem(knapsackProblem)
	fmt.Println(result)
}
